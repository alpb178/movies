...
import { saveCategoryRecipes } from '@/hooks/recipe-groups/useRecipesGroups';
import { API_CATEGORY_RECIPES_URL, DELETE } from '@/lib/constants';
...
export default function ListItemActions({ actions, resource, setLoading }) {
  ...
  const onDeleteConfirmation = async () => {
    try {
      ...
      toast(t('deleted.male', { entity: t('recipes-groups', { count: 1 }) }));
      setLoading(false);
    } catch (error) {
      toast.error(error);
    } finally {
  ...
  const onDelete = (event, resource) => {
    ...
    setDeleteConfirmationMessage({ open: true, id: resource.id });
  };
  ...
  return (
    <Menu as="div" className="relative inline-block text-left">
      ...
      <DeleteConfirmationDialog
        open={deleteConfirmationMessage.open}
        onOpen={setDeleteConfirmationMessage}
        onDeleteConfirmation={onDeleteConfirmation}
        title={t('delete-title', { entity: t('recipe-groups', { count: 1 }) })}
        content={t('delete-message.female', { entity: t('recipe-groups', { count: 1 }) })}
      />
